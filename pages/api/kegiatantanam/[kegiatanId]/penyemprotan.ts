// pages/api/kegiatantanam/[kegiatanId]/penyemprotan.ts
import type { NextApiResponse } from "next";
import mongoose from "mongoose";
import { AuthenticatedRequest, verifyToken } from "@/lib/auth";
import connectionToDatabase from "@/lib/mongodb";
import cors from "@/middleware/cors";
import { authorize } from "@/middleware/authorize";
import KegiatanTanam from "@/models/kegiatanTanam.model";
import Penyemprotan from "@/models/penyemprotan.model";
import Pestisida from "@/models/pestisida.model";

export default async function handler(
  req: AuthenticatedRequest,
  res: NextApiResponse
) {
  await cors(req, res);
  await connectionToDatabase();

  if (!(await verifyToken(req, res))) return;
  if (!(await authorize(["Petani"])(req, res))) return;

  const { kegiatanId } = req.query;
  const userId = req.user!.userId;

  if (!mongoose.Types.ObjectId.isValid(kegiatanId as string)) {
    return res.status(400).json({ error: "ID Kegiatan Tanam tidak valid" });
  }

  switch (req.method) {
    case "GET":
      try {
        const data = await Penyemprotan.find({
          plantingActivityId: kegiatanId,
          userId,
        }).populate("pesticideId", "name brand");
        return res.status(200).json(data);
      } catch (error) {
        return res
          .status(500)
          .json({ error: "Gagal mengambil riwayat penyemprotan" });
      }

    case "POST":
      const session = await mongoose.startSession();
      session.startTransaction();
      try {
        const kegiatan = await KegiatanTanam.findOne({
          _id: kegiatanId,
          userId,
        }).session(session);
        if (!kegiatan) throw new Error("Kegiatan tanam tidak ditemukan.");

        const { pesticideId, pricePerUnit, dosage, unit, date, notes } =
          req.body;
        if (
          pricePerUnit === undefined ||
          typeof pricePerUnit !== "number" ||
          dosage === undefined ||
          typeof dosage !== "number"
        ) {
          throw new Error("Harga dan dosis pestisida wajib diisi.");
        }

        const pestisida = await Pestisida.findById(pesticideId).session(
          session
        );
        if (!pestisida)
          throw new Error("Pestisida tidak ditemukan di data master.");

        if (pestisida.stock < dosage) {
          throw new Error("Stok pestisida tidak cukup.");
        }

        pestisida.stock -= dosage;
        await pestisida.save({ session });

        const newData = new Penyemprotan({
          userId,
          plantingActivityId: kegiatanId,
          pesticideId,
          pricePerUnit,
          dosage,
          unit,
          date,
          notes,
        });

        await newData.save({ session });

        // Update denormalized total cost
        kegiatan.totalCost += dosage * pricePerUnit;
        await kegiatan.save({ session });

        const populatedData = await Penyemprotan.findById(newData._id)
          .populate("pesticideId", "name brand")
          .session(session);

        await session.commitTransaction();
        return res.status(201).json(populatedData);
      } catch (error: any) {
        await session.abortTransaction();
        return res.status(400).json({
          error: "Gagal menambah data penyemprotan",
          details: error.message,
        });
      } finally {
        session.endSession();
      }

    default:
      res.setHeader("Allow", ["GET", "POST"]);
      return res
        .status(405)
        .json({ error: `Metode ${req.method} tidak diizinkan` });
  }
}
